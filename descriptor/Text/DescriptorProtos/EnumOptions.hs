...
instance P'.Mergeable EnumOptions where
  mergeEmpty = EnumOptions P'.mergeEmpty P'.mergeEmpty
  mergeAppend (EnumOptions x'1 x'2) (EnumOptions y'1 y'2) = EnumOptions (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2)
 
instance P'.Default EnumOptions where
  defaultValue = EnumOptions P'.defaultValue P'.defaultValue
 
instance P'.Wire EnumOptions where
  wireSize ft' self'@(EnumOptions x'1 x'2)
    = case ft' of
        10 -> calc'Size
  ...
  wirePut ft' self'@(EnumOptions x'1 x'2)
    = case ft' of
        10 -> put'Fields
        11
          -> do
