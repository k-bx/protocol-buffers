...
import qualified Data.ByteString.Lazy.Char8 as LC(notElem,head,readFile,uncons)
...
pOptionE = pName (U.fromString "option") >> do
...
fileOption = pOptionWith getOld >>= setOption >>= setNew >> eol where
  getOld = fmap (maybe defaultValue id . D.FileDescriptorProto.options) getState
...
messageOption = pOptionWith getOld >>= setOption >>= setNew >> eol where
  getOld = fmap (maybe defaultValue id . D.DescriptorProto.options) getState
...
subBracketOptions mt = (defaultConstant <|> fieldOptions) >> (pChar ']' <|> (pChar ',' >> subBracketOptions mt))
  where defaultConstant = do
        ...
        fieldOptions = do
          ...
          pChar '='
          (mOld,def) <- getState
          let old = maybe defaultValue id mOld
          case optName of
...
enumOption = pOptionWith getOld >>= setOption >>= setNew >> eol where
  getOld = fmap (maybe defaultValue id . D.EnumDescriptorProto.options) getState
...
enumValueOption = pOptionWith getOld >>= setOption >>= setNew >> eol where
  getOld = fmap (maybe defaultValue id . D.EnumValueDescriptorProto.options) getState
...
service = pName (U.fromString "service") >> do
 ...
 where subService = pChar '}' <|> (choice [ eol
                                          , rpc
                                          , serviceOption
                                          ] >> subService)
...
serviceOption = pOptionWith getOld >>= setOption >>= setNew >> eol where
  getOld = fmap (maybe defaultValue id . D.ServiceDescriptorProto.options) getState
...
rpcOption = pOptionWith getOld >>= setOption >>= setNew >> eol where
  getOld = fmap (maybe defaultValue id . D.MethodDescriptorProto.options) getState
...
-- There are seven of these, so just make a type class for use in this module.
class UpdateOption s where
  addUninterpreted :: D.UninterpretedOption -> s -> s

--instance UpdateOption D.FileOptions where
--  addUninterpreted u s =  s {D.FileOptions.uninterpreted_option = D.FileOptions.uninterpreted_option s |> u }

--instance UpdateOption D.MessageOptions where
--  addUninterpreted u s =  s {D.MessageOptions.uninterpreted_option = D.MessageOptions.uninterpreted_option s |> u }
instance UpdateOption D.FieldOptions where
  addUninterpreted u s =  s {D.FieldOptions.uninterpreted_option = D.FieldOptions.uninterpreted_option s |> u }
--instance UpdateOption D.EnumOptions where
--  addUninterpreted u s =  s {D.EnumOptions.uninterpreted_option = D.EnumOptions.uninterpreted_option s |> u }
-- instance UpdateOption D.EnumValueOptions where
--  addUninterpreted u s =  s {D.EnumValueOptions.uninterpreted_option = D.EnumValueOptions.uninterpreted_option s |> u }
-- instance UpdateOption D.ServiceOptions where
--  addUninterpreted u s =  s {D.ServiceOptions.uninterpreted_option = D.ServiceOptions.uninterpreted_option s |> u }
-- instance UpdateOption D.MethodOptions where
--  addUninterpreted u s =  s {D.MethodOptions.uninterpreted_option = D.MethodOptions.uninterpreted_option s |> u }
