...
instance P'.Mergeable CodeGeneratorRequest where
  ...
  mergeAppend (CodeGeneratorRequest x'1 x'2 x'3) (CodeGeneratorRequest y'1 y'2 y'3) = CodeGeneratorRequest (P'.mergeAppend x'1 y'1)
...
instance P'.Wire CodeGeneratorRequest where
  wireSize ft' self'@(CodeGeneratorRequest x'1 x'2 x'3) = case ft' of
                                                            10 -> calc'Size
                                                            11 -> P'.prependMessageSize calc'Size
                                                            _ -> P'.wireSizeErr ft' self'
    where
        calc'Size = (P'.wireSizeRep 1 9 x'1 + P'.wireSizeOpt 1 9 x'2 + P'.wireSizeRep 1 11 x'3)
  wirePut ft' self'@(CodeGeneratorRequest x'1 x'2 x'3) = case ft' of
                                                           10 -> put'Fields
                                                           11 -> do
  ...
  wireGet ft' = case ft' of
    ...
    where
        update'Self wire'Tag old'Self = case wire'Tag of
                                          10 -> P'.fmap
                                                 (\ new'Field ->
                                                   old'Self{file_to_generate = P'.append (file_to_generate old'Self) new'Field})
                                                 (P'.wireGet 9)
                                          18 -> P'.fmap (\ new'Field -> old'Self{parameter = P'.Just new'Field}) (P'.wireGet 9)
                                          122 -> P'.fmap
                                                  (\ new'Field -> old'Self{proto_file = P'.append (proto_file old'Self) new'Field})
                                                  (P'.wireGet 11)
