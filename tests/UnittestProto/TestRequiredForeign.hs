...
instance P'.Mergeable TestRequiredForeign where
  mergeEmpty = TestRequiredForeign P'.mergeEmpty P'.mergeEmpty P'.mergeEmpty
  mergeAppend (TestRequiredForeign x'1 x'2 x'3) (TestRequiredForeign y'1 y'2 y'3)
    = TestRequiredForeign (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2) (P'.mergeAppend x'3 y'3)
 
instance P'.Default TestRequiredForeign where
  defaultValue = TestRequiredForeign (P'.Just P'.defaultValue) P'.defaultValue (P'.Just P'.defaultValue)
 
instance P'.Wire TestRequiredForeign where
  wireSize ft' self'@(TestRequiredForeign x'1 x'2 x'3)
    = case ft' of
        10 -> calc'Size
        11 -> P'.prependMessageSize calc'Size
        _ -> P'.wireSizeErr ft' self'
    where
        calc'Size = (P'.wireSizeOpt 1 11 x'1 + P'.wireSizeRep 1 11 x'2 + P'.wireSizeOpt 1 5 x'3)
  wirePut ft' self'@(TestRequiredForeign x'1 x'2 x'3)
    = case ft' of
        10 -> put'Fields
        11
          -> do
