...
instance P'.UnknownMessage TestRecursiveMessage where
  getUnknownField = unknown'field
  putUnknownField u'f msg = msg{unknown'field = u'f}
 
instance P'.Mergeable TestRecursiveMessage where
  mergeEmpty = TestRecursiveMessage P'.mergeEmpty P'.mergeEmpty P'.mergeEmpty
  mergeAppend (TestRecursiveMessage x'1 x'2 x'3) (TestRecursiveMessage y'1 y'2 y'3)
    = TestRecursiveMessage (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2) (P'.mergeAppend x'3 y'3)
 
instance P'.Default TestRecursiveMessage where
  defaultValue = TestRecursiveMessage P'.defaultValue P'.defaultValue P'.defaultValue
 
instance P'.Wire TestRecursiveMessage where
  wireSize ft' self'@(TestRecursiveMessage x'1 x'2 x'3)
    = case ft' of
        10 -> calc'Size
  ...
  wirePut ft' self'@(TestRecursiveMessage x'1 x'2 x'3)
    = case ft' of
        10 -> put'Fields
        11
          -> do
